...
const headless = args.has("--headless") || process.env.IDEXALWORK_INSTALLER_HEADLESS === "1"
const dryRun = args.has("--dry-run") || process.env.IDEXALWORK_INSTALLER_DRY_RUN === "1"
...
if (headless) {
  const resolution = await resolveInstallerConfig({ installLink: argValue("--install-link") }).catch((error): never => {
    console.error(`[idexalwork-installer] ${error instanceof Error ? error.message : String(error)}`)
    process.exit(2)
  })

  const { config, source } = resolution
  console.log(`IdexalWork Installer — ${config.clientName}`)
  console.log(`[idexalwork-installer] Configured via ${installerConfigSourceLabel(source)}.`)
...
if (process.env.IDEXALWORK_INSTALLER_UI === "manual") {
  ...
  console.log(`[idexalwork-installer] UI ready at ${ready.url}`)
...
try {
  ...
  webview.title = "IdexalWork Installer"
  ...
  webview.bind("idexalworkInstallerExit", () => webview.destroy())
  webview.navigate(ready.url)
  webview.run()
  await exitWhenInstallSettles()
} catch (error) {
  // Native webview unavailable (e.g. no WebkitGTK): same UI in the browser.
  console.warn(`[idexalwork-installer] native window unavailable (${error instanceof Error ? error.message : String(error)}); opening browser UI`)
