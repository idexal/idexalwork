import { installConfigSchema, installConfigUrlFor, INSTALL_SIDECAR_FILENAME, parseInstallerFilenameTag, type InstallConfig } from "@idexalwork/install-config"
...
export class InstallerConfigMissingError extends Error {
  constructor() {
    super("Installer is not configured. Paste an IdexalWork install link, or run with --install-link <url>.")
...
function warn(options: ConfigSourceOptions | undefined, message: string) {
  ...
  logger(`[idexalwork-installer] ${message}`)
}
...
function parseConfigPayload(payload: unknown, label: string, options?: ConfigSourceOptions): InstallerConfig | null {
  ...
  if (!parsed.success) {
    warn(options, `${label} did not contain a valid IdexalWork install config.`)
    return null
  }
...
export function envOverrides(env: NodeJS.ProcessEnv = process.env): InstallerConfig | null {
  const clientName = env.IDEXALWORK_INSTALLER_CLIENT_NAME?.trim() ?? ""
  const webUrl = env.IDEXALWORK_INSTALLER_WEB_URL?.trim() ?? ""
  const apiUrl = env.IDEXALWORK_INSTALLER_API_URL?.trim() ?? ""
  const logoUrl = env.IDEXALWORK_INSTALLER_LOGO_URL?.trim() ?? ""
  const hasEnvOverride = Boolean(clientName || webUrl || apiUrl || logoUrl || env.IDEXALWORK_INSTALLER_REQUIRE_SIGNIN !== undefined)

  if (!hasEnvOverride) {
    return null
  }
  if (!clientName || !webUrl || !apiUrl) {
    throw new Error("IDEXALWORK_INSTALLER_CLIENT_NAME, IDEXALWORK_INSTALLER_WEB_URL, and IDEXALWORK_INSTALLER_API_URL are required when using installer env overrides")
  }
  ...
  return {
    ...
    requireSignin: parseRequireSignin(env.IDEXALWORK_INSTALLER_REQUIRE_SIGNIN, BUILD_REQUIRE_SIGNIN),
  }
}
