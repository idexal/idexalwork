const GITHUB_REPO = "idexal/idexalwork"
 ...
 * workflow): idexalwork-mac-<arch>-<v>.dmg, idexalwork-win-x64-<v>.exe,
 * idexalwork-linux-x86_64|arm64-<v>.AppImage — so the download URL is
...
export function releaseAssetFor(
  ...
  if (platform === "darwin") {
    return build(`idexalwork-mac-${arch}-${normalized}.dmg`, "dmg")
  }
  if (platform === "win32") {
    if (arch !== "x64") throw new Error(`unsupported Windows architecture: ${arch}`)
    return build(`idexalwork-win-x64-${normalized}.exe`, "exe")
  }
  if (platform === "linux") {
    // The AppImage uses x86_64 in its name while the tarball uses x64.
    const appImageArch = arch === "x64" ? "x86_64" : "arm64"
    return build(`idexalwork-linux-${appImageArch}-${normalized}.AppImage`, "appimage")
  }
