    ...
    async fetch(request) {
      ...
      if (url.pathname.startsWith("/api/")) {
        ...
        if (request.method === "POST" && url.pathname === "/api/resolve-link") {
          ...
          if (!installLink) {
            return Response.json({ error: "missing_install_link", message: "Paste an IdexalWork install link." }, { status: 400 })
          }
        ...
        if (request.method === "POST" && url.pathname === "/api/resolve-manual") {
          const body = await request.json().catch(() => null) as Record<string, unknown> | null
          if (!body || typeof body.webUrl !== "string" || typeof body.apiUrl !== "string") {
            return Response.json({ error: "missing_fields", message: "Web URL and API URL are required." }, { status: 400 })
          }
          resolution = {
            config: {
              clientName: "IdexalWork",
              webUrl: body.webUrl.trim(),
              apiUrl: body.apiUrl.trim(),
              logoUrl: null,
              requireSignin: true,
            },
            source: "install-link" as const,
          }
          return Response.json({ ok: true })
        }
